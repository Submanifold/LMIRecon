...
def _chamfer_distance_and_normal_consistency_single_file(file_in, file_ref, samples_per_model, num_processes=1):
  ...
  new_mesh_samples, new_mesh_normals = sample_mesh(file_in, samples_per_model)
  ref_mesh_samples, ref_mesh_normals = sample_mesh(file_ref, samples_per_model)
  ...
  ref_new_query_normals = new_mesh_normals[corr_new_ids]
  new_ref_query_normals = ref_mesh_normals[corr_ref_ids]
  ref_new_dist_sum = np.sum(ref_new_dist) / ref_new_dist.shape[0]
  new_ref_dist_sum = np.sum(new_ref_dist) / new_ref_dist.shape[0]

  chamfer_dist = ref_new_dist_sum + new_ref_dist_sum

  ##ref_mesh_normals (X, 3) to (X, 1, 3) and ref_new_query_normals (X, 3) to (X, 3, 1), then matmul
  ref_new_normal_consistency_sum = np.sum(np.abs(np.matmul(np.expand_dims(ref_mesh_normals, 1), np.expand_dims(ref_new_query_normals, 2)))) / ref_new_dist.shape[0]
  new_ref_normal_consistency_sum = np.sum(np.abs(np.matmul(np.expand_dims(new_mesh_normals, 1), np.expand_dims(new_ref_query_normals, 2)))) / new_ref_dist.shape[0]
  normal_consistency = ref_new_normal_consistency_sum
  normal_consistency_error = 1.0 - normal_consistency
  #two_way_normal_consistency = ref_new_normal_consistency_sum + new_ref_normal_consistency_sum
  return file_in, file_ref, chamfer_dist, normal_consistency_error
...
def mesh_comparison(new_meshes_dir_abs, ref_meshes_dir_abs,
  ...
  print(new_mesh_files)
  print(ref_mesh_files)
  ...
  results_chamfer_and_normal_consistency = utils_mp.start_process_pool(_chamfer_distance_and_normal_consistency_single_file, call_params, num_processes)
  results = [(r[0], r[1], str(r[2]), str(r[3])) for r in results_chamfer_and_normal_consistency]
    ...
    if new_mesh_file.split('.')[0] not in mesh_files_to_compare_set:
      if dataset_file_abs is None:
        ...
        if len(ref_mesh_files_matching) > 0:
          ...
          results.append((new_mesh_file_abs, reference_mesh_file_abs, str(-2), str(-2)))
    else:
  ...
  for ref_without_new_mesh in mesh_files_to_compare_set:
    ...
    results.append((new_mesh_file_abs, reference_mesh_file_abs, str(-1), str(-1)))

  # sort by file name
  results = sorted(results, key=lambda x: x[0])

  file_utils.make_dir_for_file(report_name)
  csv_lines = ['in mesh,ref mesh,'
               'Chamfer dist(-1: no input; -2: no reference), Normal consistency error (NCE)']
